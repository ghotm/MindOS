...
2. **发现结构**
   ...
   - 已知关键词 → 先发一条精准 `mindos_search_notes`。只有结果稀少，或主题明显存在中英文/缩写变体时，再补第二或第三条。
...
### 多文件路由
将非结构化输入拆解为离散语义单元 → 每个单元先搜最强关键词，只有需要定位最佳文件时再补充变体 → **展示路由表**（内容片段 → 目标文件 → 插入位置）→ 等用户确认 → 逐文件执行编辑 → 用文件路径汇总全部变更。
