...
import { matchesNativeRuntime, nativeRuntimeDefinition, nativeRuntimeIdForAgent } from './native-runtimes.js';
import {
...
export { isRecord } from './adapter-metadata.js';
import { isRecord } from './adapter-metadata.js';
...
export function normalizeInstalled(value: unknown): DetectedRuntimeAgent | null {
  ...
  const adapterMetadata = sanitizeAdapterMetadata(value.adapterMetadata);
  return {
    id: value.id,
    name: value.name,
...
type RuntimeAgentIdentity = Pick<DetectedRuntimeAgent | MissingRuntimeAgent, 'id' | 'name'>;

export function isCodexAgent(agent: RuntimeAgentIdentity): boolean {
  return matchesNativeRuntime(agent, nativeRuntimeDefinition('codex'));
}

export function isClaudeAgent(agent: RuntimeAgentIdentity): boolean {
  return matchesNativeRuntime(agent, nativeRuntimeDefinition('claude'));
}

export { nativeRuntimeIdForAgent };
