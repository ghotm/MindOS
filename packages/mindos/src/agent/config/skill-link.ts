...
/**
 * Skill link primitives: expose a skill body to a downstream agent by
 * creating `{agentSkillDir}/{skill}` as a symlink (junction on Windows) or,
 * when links are unavailable or the caller asks for it, as a copy carrying a
 * `.mindos-managed` marker. Shared by the Skills matrix (`handlers/skill-links.ts`),
 * `POST /api/mcp/install-skill` and the CLI bundle; the matrix read model and
 * the legacy-copy migration stay in the server handler.
 */
...
export type SkillLinkAgentMode = 'universal' | 'additional';

/** A downstream agent eligible for skill linking (present on this machine, skill-capable). */
export type SkillLinkAgent = {
  key: string;
  name: string;
  mode: SkillLinkAgentMode;
  skillDir: string;
  /**
...
export type SkillLinkStrategy = 'symlink' | 'copy';

/** Injection points so tests can simulate platform/symlink failures, plus the link strategy. */
export type SkillLinkDeps = {
  platform?: NodeJS.Platform;
  symlink?: (target: string, path: string, type: 'dir' | 'junction') => void;
  /**
   * `symlink` (default) links the body and falls back to a copy; `copy`
   * always materialises a marked copy. Installers that target npm / binary
   * runtimes use `copy`: the runtime directory moves on every update, so a
   * link into it would dangle.
   */
  strategy?: SkillLinkStrategy;
};

export type SkillCellStatus = 'linked' | 'copied' | 'broken' | 'conflict' | 'native-disabled' | 'none';
 ...
 */
export function resolveSkillSourceDir(skill: string, skillRoots: SkillRoot[]): string | null {
...
function sourceRank(root: SkillRoot): number {
...
export function statusInDir(baseDir: string, skill: string): SkillCellStatus {
...
export function cellDirsOf(agent: SkillLinkAgent): string[] {
 ...
 */
export function getSkillCellStatus(agent: SkillLinkAgent, skill: string): SkillCellStatus {
...
export function isSkillCellEnabled(status: SkillCellStatus): boolean {
...
export function linkSkillToAgent(
  ...
  try {
    ...
    if (stat?.isSymbolicLink()) {
      ...
      // A directory without SKILL.md is not a skill for any agent scanner:
      // a half-finished install we may complete, not user data to protect.
      const partial = !existsSync(join(linkPath, 'SKILL.md'));
      if (!partial && !directoriesHaveSameContent(sourceDir, linkPath)) {
        return {
          ok: false,
          code: 'conflict',
      ...
      // Pre-symlink era copy of this very skill (or a partial directory) → safe to replace in place.
 ...
 */
export function disableNativeSkill(skill: string, agent: SkillLinkAgent): SkillLinkOutcome {
  for (const dir of cellDirsOf(agent)) {
    const status = statusInDir(dir, skill);
...
function disableInDir(baseDir: string, skill: string): SkillLinkOutcome {
...
export function enableNativeSkill(skill: string, agent: SkillLinkAgent): SkillLinkOutcome {
  for (const dir of cellDirsOf(agent)) {
    const status = statusInDir(dir, skill);
...
function enableInDir(baseDir: string, skill: string): SkillLinkOutcome {
  const activePath = join(baseDir, skill);
  const disabledPath = join(baseDir, MINDOS_DISABLED_DIR, skill);
  try {
    if (!lstatSafe(disabledPath)) {
...
function isLosslessSkillCopy(skill: string, copyPath: string, skillRoots: SkillRoot[]): boolean {
...
export function createSkillLink(sourceDir: string, linkPath: string, deps: SkillLinkDeps): 'linked' | 'copied' {
  const platform = deps.platform ?? process.platform;
  const symlink = deps.symlink ?? ((target: string, path: string, type: 'dir' | 'junction') => symlinkSync(target, path, type));

  if (deps.strategy !== 'copy') {
    try {
...
export function lstatSafe(path: string): import('node:fs').Stats | null {
  try {
...
export function directoriesHaveSameContent(a: string, b: string): boolean {
