/**
 * Sunk from packages/web/lib/agent/paragraph-extract.ts (Wave 3, spec-agent-core-consolidation).
 *
...
export function extractRelevantContent(
  ...
  if (intro && intro.paragraph.length <= maxChars) {
    selected.push(intro);
    usedChars += intro.paragraph.length;
  }
