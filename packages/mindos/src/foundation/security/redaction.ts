/**
 * Secret redaction for run summaries, metadata, and event payloads.
 *
 * Foundation-level security primitive: the run ledger, knowledge audit,
 * protocols and server automations all persist through it, so it lives in
 * `foundation/security` instead of reaching into `agent/`. The former homes
 * (`agent/redaction.ts`, `agent/turn/redaction.ts`) are compatibility
 * re-exports of this single implementation (spec-runtime-lane-contract).
 */
