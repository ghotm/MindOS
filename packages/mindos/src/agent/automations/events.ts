...
/* ── Durable failure-audit port ─────────────────────────────────────────── */

/**
 * The `record-failure` audit lives in the runtime control plane, which is a
 * server-layer store; the automation event core sits in the agent layer so the
 * run ledger can emit without importing server (spec-knowledge-layering-and-
 * export-surface). The server wires the real writer at module load of
 * `server/handlers/runtime-control-plane.ts`; the registry is process-global
 * (`Symbol.for`, same reasoning as `agent/global-state.ts`) so every module
 * copy in a multi-bundle host shares one writer. Without a writer the failure
 * degrades to the console.error branch below — exactly like an audit write
 * that throws.
 */
export type AutomationFailureAuditWriter = (input: {
  mindRoot: string;
  failure: {
    id: string;
    runtimeId: string;
...
const FAILURE_AUDIT_WRITER_KEY = Symbol.for('mindos.automationFailureAuditWriter');

export function installAutomationFailureAuditWriter(writer: AutomationFailureAuditWriter | null): void {
  const registry = globalThis as unknown as Record<symbol, AutomationFailureAuditWriter | undefined>;
  if (writer) registry[FAILURE_AUDIT_WRITER_KEY] = writer;
  else delete registry[FAILURE_AUDIT_WRITER_KEY];
}

export function getAutomationFailureAuditWriter(): AutomationFailureAuditWriter | undefined {
  const registry = globalThis as unknown as Record<symbol, AutomationFailureAuditWriter | undefined>;
  return registry[FAILURE_AUDIT_WRITER_KEY];
}
...
export function recordStudioAutomationEventSourceFailure(
  ...
  try {
    const writer = getAutomationFailureAuditWriter();
    if (!writer) throw new Error('no runtime control-plane failure-audit writer is installed in this process');
    const result = writer({
      mindRoot,
      failure: {
