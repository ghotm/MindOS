/**
 * Pure agent-run timeline projection (spec-cross-process-run-events E).
 *
 * NO node imports: this module is safe inside a client bundle (Web imports it
 * through `@geminilight/mindos/server/projections/agent-run-timeline`) and on
 * the server (`/api/agent-runs?view=timeline` precomputes `timeline` with
 * it). It is the ONE implementation of the timeline visibility rules and the
 * message merge; Web and the server consume it directly, Mobile consumes the
 * server-computed `timeline` and keeps a local copy of the merge only (Metro
 * cannot import product runtime code), guarded against drift by
 * packages/mobile/__tests__/agent-run-timeline-core-parity.test.ts.
 *
 * Visibility model:
 * - `mindos-main` runs are the chat turn itself — never rendered as a card.
 * - Failed / canceled / timed-out runs and runs carrying an error are always
 *   visible: the user must see why a turn went wrong.
 * - A run with at least one ACTIONABLE event is visible. Actionable means
 *   tool / file / permission / question / error categories, run failures, or
 *   failed-ish statuses — but NOT for `native-runtime` runs, whose tool,
 *   permission and question events already render inline in the message
 *   stream (double rendering was the original bug this rule pins).
 * - `pi-subagent`, `a2a` and `mindos-headless` runs are always visible:
 *   delegated work is the point of the timeline card.
 * - `acp` runs are visible only when delegated (a parent run exists); a root
 *   ACP run IS the chat turn, like `mindos-main`.
 * - Everything else (unknown future kinds) is visible by default.
 */
...
export function isActionableTimelineEvent(event: AgentRunTimelineEvent): boolean {
...
export function isTimelineRunVisible(run: AgentRunTimelineRecord, events: AgentRunTimelineEvent[]): boolean {
...
export function selectVisibleAgentRunTimeline(input: {
  payload: { runs?: AgentRunTimelineRecord[]; events?: AgentRunTimelineEvent[] };
...
/**
 * Merges a timeline part into the newest assistant message that can host the
 * current turn. Only parts of the SAME turn (same chat session and rootRunId,
 * or same startedAfter when neither has a root) are replaced; a message can
 * legitimately carry timeline parts of older turns and keeps them.
 */
export function mergeAgentRunTimelineIntoMessages(
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    if (candidate && canReceiveTimeline(candidate, timeline)) {
      targetIndex = index;
      break;
    }
  }
...
/** Carries timeline parts across a message rebuild (stream consumer replacement). */
...
export function isSameTimelineTurn(part: MessagePart, timeline: AgentRunTimelinePart): part is AgentRunTimelinePart {
...
/** Equality that ignores `updatedAt` and record snapshots: same visible content = same part. */
